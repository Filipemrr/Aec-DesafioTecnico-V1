...
const UploadAddressDialog = ({ActualRow}) => {
  ...
  const handleOpen = () => {
    setOpen(true);
    setFormData({
      addressID: ActualRow.id,
      cep: ActualRow.cep,
      logradouro: ActualRow.logradouro,
      complemento: ActualRow.complemento,
      bairro: ActualRow.bairro,
      cidade: ActualRow.cidade,
      uf: ActualRow.uf,
      numero: ActualRow.numero,
    });
  };
  ...
  const handleSubmit = async () => {
    const { cep, logradouro, complemento, bairro, cidade, uf, numero, addressID } = formData;
    parseInt(addressID);
    ...
    const url = `${API_URL}/address/alterarEndereco`;
    const data = { cep, logradouro, complemento, bairro, cidade, uf, numero, addressID };
    const config = {
      headers: {
    ...
    try {
      ...
      setMessage(error.response?.data?.message || 'Erro ao realizar alteracoes');
      setError(true);
    }
  };
...
export default UploadAddressDialog;